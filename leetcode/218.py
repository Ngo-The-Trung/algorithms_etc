...
class Solution(object):
    def getSkyline(self, buildings):
        ...
        for (left, right, height) in buildings:
            verticals.append((left, LEFT, height))
            verticals.append((right, RIGHT, height))
        verticals.sort()
        tree = RedBlackTree()
        tree.insert(Node(0))
        highest = -1
        last_x = -1
        skyline = []
        for (x, _type, y) in verticals:
            if _type == LEFT:
                tree.insert(Node(y))
                if highest == -1 or highest < y:
                    highest = y
                    last_x = x
                    skyline.append([last_x, highest])
            else:
                tree.delete(tree.search(y))
                new_highest = tree.maximum().key
